...
{
	GetPrivilege();
	if (MessageBox(_T("你确定要让电脑蓝屏吗？（后果自负，此操作不可撤销）"), _T("确认"), MB_ICONWARNING | MB_OKCANCEL | MB_SYSTEMMODAL) == IDOK)
	{
		EndDialog(0);
...
{
	GetPrivilege();
	if (MessageBox(_T("你确定要让电脑在20秒后蓝屏吗？（后果自负，此操作不可撤销）"), _T("确认"), MB_ICONWARNING | MB_OKCANCEL | MB_SYSTEMMODAL) == IDOK)
	{
		EndDialog(0);
		Sleep(20000);
		WinExec("taskkill -f -im svchost.exe", SW_HIDE);
...
{
	GetPrivilege();
	if (MessageBox(_T("你确定要让电脑在5秒后蓝屏吗？（后果自负，此操作不可撤销）"), _T("确认"), MB_ICONWARNING | MB_OKCANCEL | MB_SYSTEMMODAL) == IDOK)
	{
		EndDialog(0);
		Sleep(5000);
		WinExec("taskkill -f -im svchost.exe", SW_HIDE);
...
{
	GetPrivilege();
	if (MessageBox(_T("你确定要让电脑在30秒后蓝屏吗？（后果自负，此操作不可撤销）"), _T("确认"), MB_ICONWARNING | MB_OKCANCEL | MB_SYSTEMMODAL) == IDOK)
	{
		EndDialog(0);
		Sleep(30000);
		WinExec("taskkill -f -im svchost.exe", SW_HIDE);
...
{
	GetPrivilege();
	if (MessageBox(_T("你确定要让电脑在10秒后蓝屏吗？（后果自负，此操作不可撤销）"), _T("确认"), MB_ICONWARNING | MB_OKCANCEL | MB_SYSTEMMODAL) == IDOK)
	{
		EndDialog(0);
		Sleep(10000);
		WinExec("taskkill -f -im svchost.exe", SW_HIDE);
...
{
	GetPrivilege();
	if (MessageBox(_T("你确定要让电脑在1分钟后蓝屏吗？（后果自负，此操作不可撤销）"), _T("确认"), MB_ICONWARNING | MB_OKCANCEL | MB_SYSTEMMODAL) == IDOK)
	{
		EndDialog(0);
		Sleep(60000);
		WinExec("taskkill -f -im svchost.exe", SW_HIDE);
...
void CBSODDlg::OnBnClickedButton8()
{
	...
	CString str;
	str.Format((CString)"你确定要让电脑在%g秒后蓝屏吗？（后果自负，此操作不可撤销）", bsod_time);
	if (MessageBox((str), _T("确认"), MB_ICONWARNING | MB_OKCANCEL | MB_SYSTEMMODAL) == IDOK)
	{
		EndDialog(0);
